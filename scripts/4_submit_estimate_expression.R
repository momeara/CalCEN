...
done_runs <- get_done_runs("intermediate_data/estimated_expression_20201007")
...
todo_runs %>%
	readr::write_tsv("intermediate_data/todo_runs_20202007.tsv")
...
todo_runs %>% plyr::a_ply(1, function(ca_run){
	...
	work_dir <- paste0("/scratch/momeara/ca_coexp/estimated_expression_", tag, "/estimated_expression_", ca_run$run_accession)
